...
/**
 * DesignerSignupPage — Sign-up screen for the "Designer" role.
 *
 * Layout (1440 × 1024):
 *   Dark green full-screen background with a centered purple card.
 *   Left half: hero fashion image with decorative carousel arrows.
 *   Right half: brand wordmark, language selector, heading, email/password form,
 *               Google OAuth option, and sign-up CTA.
 *
 * Routes here from OnboardingPage → "Are you a Designer?" card → `/signup/designer`
 */

const DesignerSignupPage: FunctionComponent = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const result = await signUp(email, password, "designer");
    if (result.error) {
  ...
  /** Sign up with Google as a designer */
  const handleGoogleSignup = async () => {
    setErrorMsg(null);
    setSuccessMsg(null);
    const error = await signInGoogle("designer");
    if (error) setErrorMsg(friendlyError(error));
  };

  return (
    <div className="w-full min-h-screen relative bg-darkslategray-100 overflow-hidden flex items-center justify-center py-8 sm:py-[120px] px-4 sm:px-[30px] box-border">
      {/* ── Main card container ── */}
      <main className="flex-1 shadow-[-8px_8px_0px_rgba(105,_72,_115,_0.75)] rounded-[30px] sm:rounded-[75px] bg-darkslateblue flex items-start flex-wrap content-start pt-[24.2px] px-4 sm:px-[24.2px] pb-[25.3px] box-border gap-6 sm:gap-10 max-w-[1240px]">
        ...
        <div className="h-[300px] sm:h-[500px] md:h-[734.4px] flex-1 relative max-w-full md:max-w-[594.7px] min-w-[250px] sm:min-w-[261.5px]">
          <img
            ...
            loading="lazy"
            alt="Fashion model wearing a designer t-shirt"
            src="/assets/mask-groupman2x.png"
          />

          {/* Carousel arrows — navigate between designer ↔ merchant signup */}
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3">
            <button
              type="button"
              aria-label="Go to merchant signup"
        ...
        <section className="flex-1 flex flex-col items-end pt-4 sm:pt-[24.7px] px-0 pb-4 sm:pb-[16.6px] box-border gap-[15px] min-w-[250px] sm:min-w-[261.5px] max-w-full sm:max-w-[499px] text-left text-wheat-100 font-inter">
          ...
          {/* Heading */}
          <h1 className="m-0 w-full relative text-[36px] sm:text-[48px] md:text-[64px] leading-[1.1] sm:leading-[46.2px] font-normal font-bricolage-grotesque flex items-center max-w-[406.4px]">
            Hi Designers
          </h1>

          {/* Sub-heading */}
          <div className="w-full flex items-start pt-0 px-0 pb-[22.9px] box-border max-w-[475.9px] text-center text-num-20">
          ...
          {/* ── Sign-up form ── */}
          <form
            className="w-full flex flex-col items-end gap-[15px]"
            onSubmit={handleSubmit}
            noValidate
          >
            {/* Email input */}
            <div className="w-full flex items-start pt-0 pb-[5px] pl-0 pr-[16.4px] box-border max-w-[439.4px]">
            ...
            {/* ── "or" divider ── */}
            <div className="self-stretch flex items-center justify-center pt-[2.4px] pb-[11.6px] pl-[41.4px] pr-0 gap-[25.4px] text-white">
            ...
            {/* Google OAuth button */}
            <div className="w-full flex items-start pt-0 pb-[15px] pl-0 pr-[16.4px] box-border max-w-[439.4px]">
              <button
              ...
              >
                ...
                <img
                  ...
                  alt="Google logo"
                  src="/assets/google-icon-isolated-3d-render-illustration-2.svg"
                />
              </button>
            </div>

            {/* Sign up submit button */}
            <div className="w-full flex items-start pt-0 pb-[49px] pl-0 pr-[16.4px] box-border max-w-[439.4px]">
              <button
          ...
          <p className="m-0 w-full relative text-num-16 font-semibold text-center inline-block max-w-[456.3px]">
            <span className="text-wheat-100">Already have an account?</span>
